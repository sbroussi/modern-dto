...
/**
 * A simple sender that send a message and read a response on JMS queues.
 */
@Slf4j
public class SenderJms implements MessageSender {
    ...
    public SenderJms(final QueueConnectionFactory factory,
    ...
    private String sendMessage(final boolean isOneWayRequest,
        ...
        try {
            ...
            if ((correlationId == null) || (correlationId.trim().length() == 0)) {
                throw new TransportException("Expected a response but no correlationId is returned");
            }
            ...
            if (jmsMessage == null) {
                throw new TransportTimeoutException("No reply message with correlationId [" + correlationId
                        + "]. Probably timed-out (waited " + timeout + " ms)");
            }
            if (!(jmsMessage instanceof TextMessage)) {
                throw new TransportException("Expected a 'TextMessage' but received a ["
            ...
            throw new TransportException("cannot send JMS message [" + rawMessage + "]", t);
        }
        return rawResponse;
    }
}
